...
class WeatherWidget(FloatLayout):
    ...
    current_station = ObjectProperty()
    current_title = ObjectProperty()
    ...
    def radioConnect(self):
        try:
            ...
            mpdclient.repeat(1) # skip 'next' at the end of the playlist jumps to the first entry
            mpdclient.consume(0) # do not remove songs (stations) from the playlist
        except Exception as ex:
            #print(f"WebRadio: radioConnect() Exception {ex=}")
            pass

    def radioUpdateStatus(self):
        #print('radioUpdateStatus()')
        self.radioConnect()

        try:
            self.radiostatus = mpdclient.status()
            self.currentsong = mpdclient.currentsong()
            #print(f'status: {self.radiostatus}')
            #print(f'playlist: {mpdclient.playlist()}')
            #print(f'playlistinfo   : {mpdclient.playlistinfo()}')
            #print(f"status.state   : {self.radiostatus['state']}")
            #print(f"current station: {self.currentsong['name']}")
            #print(f"current song   : {self.currentsong['title']}")
            if self.radiostatus['state'] == 'play':
                self.current_station.text = self.currentsong['name']
                self.current_title.text = self.currentsong['title']
            else:
                self.current_station.text = ''
                self.current_title.text = ''
            # TODO publish volume to mqtt
            #publish('webradio/volume', self.radiostatus['volume'])

        except Exception as ex:
            #print(f"WebRadio: radioUpdateStatus() Exception {ex=}")
            pass


    def radioPlayPause(self):
        print('radioPlayPause()')
        self.radioConnect()
        if self.radiostatus is None:
            pass
        if self.radiostatus['state'] == 'play':
            mpdclient.pause(1)
        else:
    ...
    def radioSetVolume(self, vol):
        print('radioSetVolume()')
        self.radioConnect()
        mpdclient.setvol(vol)

    def radioVolumeUp(self):
        print('radioVolumeUp()')
        self.radioConnect()
        mpdclient.volume(10)

    def radioVolumeDown(self):
        print('radioVolumeDown()')
        self.radioConnect()
        mpdclient.volume(-10)

    def radioPlayPrev(self):
        print('radioPlayPrev()')
        self.radioConnect()
        try:
            mpdclient.previous()
        except Exception as ex:
            print(f"WebRadio: prev() Exception {ex=}")
            pass

    def radioPlayNext(self):
        print(f'radioPlayNext() last_playnext_timestamp = {self.last_playnext_timestamp}')
        if time.time() - self.last_playnext_timestamp < 4:
            print('ignore double playnext')
            return
        self.last_playnext_timestamp = time.time()
        self.radioConnect()
        try:
            print('next()')
            mpdclient.next()
        except Exception as ex:
            print(f"WebRadio: next() Exception {ex=}")
            pass
    ...
    def on_release_focus(self):
        ...
        if self.clock_update_timer is not None:
            self.clock_update_timer.finish()
        del self.clock_update_timer
        self.clock_update_timer = None
        if self.weather_update_timer is not None:
    ...
    def on_mqtt_message(self, message):
        ...
        try:
            ...
            if message.topic == 'webradio/play':
                print(message.topic, ': ', payload)
                self.radioPlayPause()
                #try:
                #    output = subprocess.run(['/usr/bin/mpc', 'toggle'], stdout=subprocess.PIPE).stdout.decode('utf-8')
                #except Exception as err:
                #    print(f'Exception: {err=}')

            if message.topic == 'webradio/pause':
                print(message.topic, ': ', payload)
                try:
                    output = subprocess.run(['/usr/bin/mpc', 'toggle'], stdout=subprocess.PIPE).stdout.decode('utf-8')
                except Exception as err:
            ...
            if message.topic == 'webradio/next':
                print(message.topic, ': ', payload)
                try:
                    output = subprocess.run(['/usr/bin/mpc', 'next'], stdout=subprocess.PIPE).stdout.decode('utf-8')
                except Exception as err:
                    print(f'Exception: {err=}')

            if message.topic == 'webradio/volup':
                print(message.topic, ': ', payload)
                self.radioVolumeUp()
                #try:
                #    output = subprocess.run(['/usr/bin/mpc', 'volume', '70'], stdout=subprocess.PIPE).stdout.decode('utf-8')
                #except Exception as err:
                #    print(f'Exception: {err=}')
            if message.topic == 'webradio/voldown':
                print(message.topic, ': ', payload)
                self.radioVolumeDown()
                #try:
                #    output = subprocess.run(['/usr/bin/mpc', 'volume', '50'], stdout=subprocess.PIPE).stdout.decode('utf-8')
                #except Exception as err:
                #    print(f'Exception: {err=}')

            if message.topic == 'webradio/setvolume':
                print(message.topic, ': ', payload)
                self.radioSetVolume(payload)

            if message.topic == 'webradio/setstation':
                print(message.topic, ': ', payload)
                try:
